...
#include "PluginLog.h"
///Define local PLOG output.  the calling plugin would need to pass its plugin pointer down in a constructor
//if we wanted to upload this to the cloud and link to plugin.
//If we want to just write this level only to file, better to #define FILE_LOG variant instead.
#define PLOG_I(level) PLUGIN_LOG("Intersection", level, 0) //pass 0 as plugin pointer, not fixed to plugin
