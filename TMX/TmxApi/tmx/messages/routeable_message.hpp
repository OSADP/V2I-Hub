...
private:
	/**
	 * Create a message from a C-style string representation of the contents.  This function is used as
	 * an intermediate stage from the (old) IVP Message structure to the new ptree representation.  Because
	 * that C-style string needs cleaned up, a free is done after construction, and doing that generally
	 * would be dangerous.  Therefore, this constructor is private, and should not be used outside the
	 * original intent.  Using the C++ string representation of the contents is sufficient for other cases.
	 * @see tmx::routeable_message(const std::string &)
	 * @param contents A C-style string representation of the contents
	 */
	tmx_routeable_message(const char *contents): tmx_message<Format>(std::string(contents))
	{
		free((char *)contents);
	}
public:
	...
	tmx_routeable_message(IvpMessage *other):
		tmx_routeable_message<Format>(ivpMsg_createJsonString(other, IvpMsg_FormatOptions_none)) { }
