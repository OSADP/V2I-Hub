...
// Add Connects to
	...
	if (geomMapGroup->lane[laneIter].type == computed) {
		...
		lane->nodeList.choice.computed.offsetXaxis.choice.small =
				geomMapGroup->lane[laneIter].referencelane.xoffset * 10;
		...
		lane->nodeList.choice.computed.offsetYaxis.choice.large =
				geomMapGroup->lane[laneIter].referencelane.yoffset * 10;
	} else {
		...
		for (n = 0; n < map_maxnodes; n++) {
			...
			int modX = geomMapGroup->lane[laneIter].node[n].eastern * 10;
			int modY = geomMapGroup->lane[laneIter].node[n].northern * 10;
			int largestOffset;
...
void ConvertToJ2735r41::convertRoadwayMap(MapData* MapJ2735, map* mapMessage) {
	...
	*(MapJ2735->layerType) = LayerType_roadwaySectionData;
	...
	for (g = 0; g < map_maxgeometries; g++) {
		...
		// TODO: Include Version from XML - Not defined in structure but it is in the message
		roadSegment->revision = (MsgCount_t) (1);
...
int ConvertToJ2735r41::convertMap(MapData *MapJ2735, map *mapMessage,
	...
	asn_enc_rval_t rvalenc = uper_encode_to_buffer(&asn_DEF_MapData, MapJ2735,
			encodedBytes, 4000);
...
//	std::cout << "msgID:" << frame->msgID << " contentID:" << frame->contentID << std::endl;
	...
	asn_enc_rval_t encResults = der_encode_to_buffer(&asn_DEF_UPERframe, frame,
			&derEncoded, 4000);
	if (encResults.encoded == 0) {
		std::cout << "Did not Successfully DER encode message" << std::endl;
		return false;
	} else {
		std::cout << "Mape Successfully DER encoded:" << encResults.encoded
				<< std::endl;
	...
	printf("MAP successfully DER encoded size: %d\nPayload: ",
			derEncodedByteCount);
	for (i = 0; i < derEncodedByteCount; i++) {
		printf("%02x ", derEncoded[i]);
	}
	printf("\n");
