...
function getSystemName($con)
{
	$host = gethostname();
	$query = "SELECT * FROM `systemConfigurationParameter` WHERE `systemConfigurationParameter`.`key`='SYSTEM_NAME'";
	$result = mysqli_query($con, $query);
	if (mysqli_errno($con) == '')
	{
...
<tr><td>+ <a href="<?php echo($c_PathPrefix) ?>/admin/system.php">Manage System</a></td></tr>
<?php } ?>
<?php if($_SESSION['accessLevel'] >= 2) { ?>
