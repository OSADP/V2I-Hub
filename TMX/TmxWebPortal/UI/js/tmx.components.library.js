...
VerticalMeter.prototype.onmousedown = function (event) {
    if (document.getElementById('popupMenu') != null && document.getElementById('popupMenu') != undefined) {
        ...
        /*
        if (this.currentVal == "ENABLED") $("#popupSwdAlarm").html("Disable Alarm").enhanceWithin();
        else $("#popupSwdAlarm").html("Enable Alarm").enhanceWithin();
        */
        var y_distance = event.y;
        if (y_distance >= 0.65 * screen.height) { y_distance *= 0.8;
        }
...
// Telemetry display for DPT and DPR ---------------------------------------------------------------
TelemDprDptDisplay = function (component) {
    var node = null;
    var obj = this;
    var currentVal;
    var currentAlarm;
...
TelemDprDptDisplay.prototype.onmousedown = function (event) {
    if (document.getElementById('popupMenu') != null && document.getElementById('popupMenu') != undefined) {
        var popup = this.node.getAttribute('data-nopopup');
        if (popup == "true") {
            return;
...
/*
TelemDprDptDisplay.mousedown = function() {
  console.log( "Handler for .mousedown() called." );
};
*/
TelemDprDptDisplay.prototype.refresh = function (val) {
    currentVal = val;
    ...
    if (val == "0" || val == 0) {
        this.node.querySelector('.templateValue').innerHTML = "Manual";
    } else if (val == "1" || val == 1) {
        this.node.querySelector('.templateValue').innerHTML = "Main";
    } else if (val == "2" || val == 2) {
        this.node.querySelector('.templateValue').innerHTML = "Aux";
    }
};

TelemDprDptDisplay.prototype.refreshAlarm = function (val) {
    if (val == "DISABLED" || val == "0") {
        this.node.querySelector('.templateLabel').style.color = "orange";
        this.node.setAttribute("data-alarmstate", "disabled");
    } else if (val == "ENABLED" || val == "1") {
...
TelemDprDptDisplay.prototype.refreshMode = function (val) {
    this.node.setAttribute('data-mode', val);
    //if (this.node.querySelector('.templateValue').innerHTML != val)
    //	this.node.querySelector('.templateValue').innerHTML = val; 
};

TelemDprDptDisplay.prototype.limit = function (newVal) {
    if ((newVal == "RED_LIMIT_HIGH") || (newVal == "RED_LIMIT_LOW")) {
        this.node.querySelector('.templateValue').style.background = "#ff0000";
        this.node.querySelector('.templateValue').style.color = "#2e2e2e";
    }
...
TelemDprDptDisplay.prototype.name = function (name, replacement) {
    var replacing = this.node.querySelector('.templateLabel');
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
...
TelemDprDptDisplay.prototype.units = function (val) {
    var units = this.node.querySelector('.templateUnits');
    if (units != null && units != undefined) {
        units.innerHTML = val;
    }
...
GroundTable = function (component) {
    var node = null;
    var obj = this;
    var currentVal;
    var currentAlarm;
...
GroundTable.prototype.onmousedown = function (event) {
};


/*
GroundTable.mousedown = function() {
  console.log( "Handler for .mousedown() called." );
};
*/
GroundTable.prototype.refresh = function (val) {
    ...
    if (this.node.className.indexOf("gndTable") != -1) {
        var gndItems = document.getElementsByClassName("gndTable");
        if (gndItems.length < 3) {
            // Not enough items for the ground table
            return;
        }
        var i = 0;
        do {
            var lowest = document.getElementById('firstLowestGnd');
            var second_lowest = document.getElementById('secondLowestGnd');
            var third_lowest = document.getElementById('thirdLowestGnd');

            if (lowest == null || lowest == undefined            
                || second_lowest == null || second_lowest == undefined
                || third_lowest == null || third_lowest == undefined) {

                if (lowest != null && lowest != undefined) {
                    $("#firstLowestGnd").addClass("hidden");
                    lowest.id = "";
                }

                if (second_lowest != null && second_lowest != undefined) {
                    $("#secondLowestGnd").addClass("hidden");
                    second_lowest.id = "";
                }

                if (third_lowest != null && third_lowest != undefined) {
                    $("#thirdLowestGnd").addClass("hidden");
                    third_lowest.id = "";
                }

                lowest = gndItems[0];
                gndItems[0].id = "firstLowestGnd";

                second_lowest = gndItems[1];
                gndItems[1].id = "secondLowestGnd";

                third_lowest = gndItems[2];
                gndItems[2].id = "thirdLowestGnd";
                //i = 0;
                $('#firstLowestGnd').removeClass('hidden');
                $('#secondLowestGnd').removeClass('hidden');
                $('#thirdLowestGnd').removeClass('hidden');
            }

            var lowest_sensor = lowest.getAttribute('data-sensor');
            var second_sensor = second_lowest.getAttribute('data-sensor');
            var third_sensor = third_lowest.getAttribute('data-sensor');
            ...
            var newVal = gndItems[i].querySelector('.templateValue').innerHTML;
            newVal = parseFloat(newVal);
            var sensor = obj.getAttribute("data-sensor");
            switchTelem = false;

            // Item is "---", so lowest priority; don't need to check if there are any lower
            if (isNaN(newVal)) {
                i++;
                continue;
            }

            if (sensor != lowest_sensor) {
                var tempVal = lowest.querySelector('.templateValue').innerHTML;
                tempVal = parseFloat(tempVal);

                // First lowest gnd is "---", which is lowest priority
                if (isNaN(tempVal) || newVal < tempVal) {
                    $('#thirdLowestGnd').addClass('hidden');
                    third_lowest.id = "";
                    second_lowest.id = "";
                    lowest.id = "";
                    second_lowest.id = "thirdLowestGnd";
                    lowest.id = "secondLowestGnd";
                    obj.id = "firstLowestGnd";
                    ...
                    lowest_sensor = lowest.getAttribute('data-sensor');
                    second_sensor = second_lowest.getAttribute('data-sensor');
                    third_sensor = third_lowest.getAttribute('data-sensor');
                }
            }
            if (sensor != second_sensor && sensor != lowest_sensor) {
                var tempVal = second_lowest.querySelector('.templateValue').innerHTML;
                tempVal = parseFloat(tempVal);
            ...
            if (sensor != third_sensor && sensor != lowest_sensor && sensor != second_sensor) {
                var tempVal = third_lowest.querySelector('.templateValue').innerHTML;
                tempVal = parseFloat(tempVal);

                if (isNaN(tempVal) || newVal < tempVal) {
                    $('#thirdLowestGnd').addClass('hidden');
                    third_lowest.id = "";
                    obj.id = "thirdLowestGnd";
            ...
            if (i == gndItems.length - 1) {
                break;
            }

            do {
                i++;
            } while (i < (gndItems.length - 1) && gndItems[i].id.length != 0);

        }
        while (i < gndItems.length);
        /*
        $("#firstLowestGnd").removeClass("hidden");
        $("#secondLowestGnd").removeClass("hidden");
        $("#thirdLowestGnd").removeClass("hidden");
...
GroundTable.prototype.refreshAlarm = function (val) {
    if (val == "DISABLED" || val == "0") {
        this.node.querySelector('.templateLabel').style.color = "orange";
        this.node.setAttribute("data-alarmstate", "disabled");
    } else if (val == "ENABLED" || val == "1") {
...
GroundTable.prototype.refreshMode = function (val) {
    this.node.setAttribute('data-mode', val);
    //if (this.node.querySelector('.templateValue').innerHTML != val)
    //	this.node.querySelector('.templateValue').innerHTML = val; 
};

GroundTable.prototype.limit = function (newVal) {
    if ((newVal == "RED_LIMIT_HIGH") || (newVal == "RED_LIMIT_LOW")) {
        this.node.querySelector('.templateValue').style.background = "#ff0000";
        this.node.querySelector('.templateValue').style.color = "#2e2e2e";
    }
...
GroundTable.prototype.name = function (name, replacement) {
    var replacing = this.node.querySelector('.templateLabel');
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
...
GroundTable.prototype.units = function (val) {
    var units = this.node.querySelector('.templateUnits');
    if (units != null && units != undefined) {
        units.innerHTML = val;
    }
...
// Telemetry Button display ----------------------------------------------------------------------
 TelemButton = function (component) {
     var node = null;
     var obj = this;
     var currentVal;
     var currentAlarm;
...
TelemButton.prototype.onmousedown = function (event) {
};

TelemButton.prototype.refresh = function (val) {
    currentVal = val;
    if (val == "reset_display") val = "---"
    if (this.node.innerHTML != val)
        this.node.innerHTML = val;
};

TelemButton.prototype.refreshAlarm = function (val) {};

TelemButton.prototype.refreshMode = function (val) { };

TelemButton.prototype.limit = function (newVal) {
    if ((newVal == "RED_LIMIT_HIGH") || (newVal == "RED_LIMIT_LOW")) {
        this.node.style.background = "#ff0000";
        this.node.style.color = "#2e2e2e";
    }
    else if ((newVal == "YELLOW_LIMIT_HIGH") || (newVal == "YELLOW_LIMIT_LOW")) {
        this.node.style.background = "#ffff00";
        this.node.style.color = "#2e2e2e";
    }
    else {
        this.node.style.background = "#2e2e2e";
        this.node.style.color = "#f9f9f9";
    }
};

TelemButton.prototype.name = function (name, replacement) {
    var replacing = null; //TODO
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
    }
...
TelemButton.prototype.units = function (val) {
...
LedDisplay.prototype.refresh = function (val) {
    ...
    else if ((this.type == "swd") && (this.node.getAttribute("data-device") != "SMC")) {
         if ((value == "1") || (value == "true") || (value == "alarm") || (value == 1)) {
            if (this.node.getAttribute("data-state") != "red")
                this.node.setAttribute("data-state", "red");
        }
        else if ((value == "2") || (value == "warning") || (value == 2)) {
            if (this.node.getAttribute("data-state") != "yellow")
                this.node.setAttribute("data-state", "yellow");
        }
...
{
    if ((this.type == "swd") && (this.node.getAttribute("data-device") == "SMC")) {
        if ((newVal == "RED_LIMIT_HIGH") || (newVal == "RED_LIMIT_LOW")) {
            if (this.node.getAttribute("data-state") != "red")
                this.node.setAttribute("data-state", "red");
        }
...
//------------------------------------------------------------------------------------------------------------------
InverseSwdDisplay = function (component) {
    var node;
    var type;
    var auto;
    var obj = this;
...
InverseSwdDisplay.prototype.refresh = function (val) {
    value = val.toLowerCase();
    var prevValue = this.node.getAttribute("data-state");
    //		console.log("Refresh Value:" + value);
    if ((value == 0)) {
...
InverseSwdDisplay.prototype.refreshAlarm = function (val) {
    value = val.toLowerCase();
    if ((value == 0)) {
        if (this.node.getAttribute("data-state") != "green")
            this.node.setAttribute("data-state", "green");
...
InverseSwdDisplay.prototype.refreshMode = function (val) {
    this.node.setAttribute('data-mode', val);
};


InverseSwdDisplay.prototype.getValue = function () {
    return "";
};
InverseSwdDisplay.prototype.limit = function (newVal) { };
InverseSwdDisplay.prototype.name = function (name, replacement) {
    var replacing = null;
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
...
InverseSwdDisplay.prototype.units = function (val) { };
...
// -----------------------------------------------------------------------------
ModuleButton = function (component, num) {
    var node = null;
    var obj = this;
    var currentVal = 1;
    var buttonID = null;
    obj.buttonID = "moduleButton" + num;
    obj.node = component;
    ...
    obj.node.innerHTML += (
                            "<button id= 'moduleButton" + num + "' class='module' onmousedown=switchPage('" + obj.node.getAttribute("data-page") + "')>" +
                                "<div class='modulelabel'>" + obj.node.getAttribute("data-label") + "</div>" +
                                "<div class='ledbar' data-led-type='module' data-state='disabled' data-value='---'' data-units='' data-sensor='" + obj.node.getAttribute("data-sensor") + "' data-device='" + obj.node.getAttribute("data-device") + "'>" +
                                "<div class='modelabel'>???</div>" +
                                "<div class='alarmStatelabel'></div>" +
                            "</button>"
    );
...
ModuleButton.prototype.refresh = function(val) 
{

};

ModuleButton.prototype.refreshAlarm = function (val) {
    if (val == "DISABLED" || val == "0") {
        this.node.querySelector('.modulelabel').style.color = "orange";
    } else if (val == "ENABLED" || val == "1") {
        this.node.querySelector('.modulelabel').style.color = "#FFFFFF";
    }
};

ModuleButton.prototype.refreshMode = function (val) {

    this.node.setAttribute('data-mode', val);
    if (val == "AUTOMATIC") {
        if (this.node.querySelector('.modelabel').innerHTML != "Auto") //"Automatic")
            this.node.querySelector('.modelabel').innerHTML = "Auto";  //"Automatic";
    }
    else if (val == "MANUAL") {
        if (this.node.querySelector('.modelabel').innerHTML != "Man") //"Manual")
            this.node.querySelector('.modelabel').innerHTML = "Man";  //"Manual";
    } else if (val == "SW_MANUAL") {
        if (this.node.querySelector('.modelabel').innerHTML != "SW Man") //"SW Manual")
            this.node.querySelector('.modelabel').innerHTML = "SW Man";  //"SW Manual";
    } else
    {
//        if (this.node.querySelector('.modelabel').innerHTML != "???")
//            this.node.querySelector('.modelabel').innerHTML = "???";        
        if (this.node.querySelector('.modelabel').innerHTML != val)
            this.node.querySelector('.modelabel').innerHTML = val;        
    }
};


ModuleButton.prototype.limit = function(newVal) 
{
};

ModuleButton.prototype.name = function (name, replacement) {
    var replacing = this.node.querySelector('.modulelabel');
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
    }
};
ModuleButton.prototype.units = function (val) { };
// -----------------------------------------------------------------------------
SwdDisplay = function (component) {
    var node = null;
    var obj = this;
    var currentVal;
    obj.node = component;

    var alarmIncludedFlag = (obj.node.getAttribute("data-alarmState") != null) && (obj.node.getAttribute("data-alarmState") != null);
    /*
    var state = 'disabled';
    if (alarmIncludedFlag) {
    state = obj.node.getAttribute('data-alarmState');
    } else {
    state = obj.node.getAttribute('data-state');
    }
    */
    var html = "<div class='templateValue padding-0_4em'>" +
                    "<div class='led inline-block vertical-align-middle' data-state='disabled' data-sensor='" + obj.node.getAttribute("data-sensor") + "' data-device='" + obj.node.getAttribute("data-device") + "' data-led-type='swd' data-mode='" + obj.node.getAttribute('data-mode') + "'></div>";

    html += "<div class='swdLabel'>" + obj.node.getAttribute("data-label") + "</div>" +
                "</div>";

    //    console.log(html);
    this.currentVal = "ENABLED";
    obj.node.innerHTML += (html);
    obj.node.addEventListener('mousedown', function () {
        obj.onmousedown(event);
    }, true);
    ...
    if ((obj.node.getAttribute("data-device") != null) && (obj.node.getAttribute("data-sensor") != null)) {
        ...
        if (!checkAlarmHandlerForElement(obj.node)) {
            // TODO: Need to register this component for ALARM_STATE Message;
            //registerObject(createTelemObject(obj, obj.node.getAttribute("data-device"), obj.node.getAttribute("data-sensor"), updateCustom, updateCustomNoData, updateMode));
            if (alarmIncludedFlag) {
                registerAlarmObject(createAlarmObject(obj, obj.node.getAttribute("data-device"), obj.node.getAttribute("data-sensor"), updateAlarm, updateCustomNoData, updateMode));
            }
        }
...
SwdDisplay.prototype.onmousedown = function (event) {
    if (document.getElementById('swdPopupMenu') != null && document.getElementById('swdPopupMenu') != undefined) {
        var popup = this.node.getAttribute('data-nopopup');
        if (popup == "true") {
            return;
        }
        $("#swdPopupMenuHeader").html(this.node.getAttribute("data-label")).enhanceWithin();
        $("#swdPopupMenu").attr("data-device", this.node.getAttribute("data-device"));
        $("#swdPopupMenu").attr("data-sensor", this.node.getAttribute("data-sensor"));

        if (this.node.getAttribute('data-alarmState') == "enabled") {
            document.getElementById('popupSwdAlarm').innerHTML = "Disable Alarm";
        } else if (this.node.getAttribute('data-alarmState') == "disabled") {
            document.getElementById('popupSwdAlarm').innerHTML = "Enable Alarm";
        }

        var y_distance = event.y;
        if (y_distance >= 0.65 * screen.height) { y_distance *= 0.8; }
        $("#swdPopupMenu").popup("open", { x: event.x, y: y_distance });
    }
};

SwdDisplay.prototype.refresh = function(val) 
{
    if (this.currentVal != val)
        this.currentVal = val;
};
SwdDisplay.prototype.refreshAlarm = function(val) {
//    console.log("Alarm:" + this.node.getAttribute("data-sensor") + " - " + val);
    if (val == "DISABLED" || val == "0") {
        this.node.querySelector('.swdLabel').style.color = "orange";
        this.node.setAttribute("data-alarmstate", "disabled");
    } else if (val == "ENABLED" || val == "1") {
        this.node.querySelector('.swdLabel').style.color = "#FFFFFF";
        this.node.setAttribute("data-alarmstate", "enabled");
    }
};

SwdDisplay.prototype.refreshMode = function(val) 
{
    this.node.setAttribute('data-mode', val);
};


SwdDisplay.prototype.limit = function (newVal) {
};

SwdDisplay.prototype.name = function (name, replacement) {
    var replacing = this.node.querySelector('.swdLabel');
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
    }
}
SwdDisplay.prototype.units = function (val) {};

// Telemetry display ----------------------------------------------------------------------
TelemClockSourceDisplay = function (component) {
    var node = null;
    var obj = this;
    var currentVal;
    var currentAlarm;
    ...
    newHTML += "<div class='templateValue'>" + obj.node.getAttribute("data-value") + "</div>";
    if ((obj.node.getAttribute("data-units") != "") && (obj.node.getAttribute("data-units") != null)) {
        newHTML += "<span class='templateUnits'>" + obj.node.getAttribute("data-units") + "</span>";
    }
    obj.node.innerHTML += (newHTML);
...
TelemClockSourceDisplay.prototype.onmousedown = function (event) {
};
...
*/
TelemClockSourceDisplay.prototype.refresh = function (val) {
    if (val == "reset_display" || val == "---") {
        this.node.querySelector('.templateValue').innerHTML = "SYS";
    } else {
        this.node.querySelector(".templateValue").innerHTML = "NTC";
    }
};

TelemClockSourceDisplay.prototype.refreshAlarm = function(val) 
{
};

TelemClockSourceDisplay.prototype.refreshMode = function (val) {
    this.node.setAttribute('data-mode', val);
};

TelemClockSourceDisplay.prototype.limit = function(newVal) 
{
};

TelemClockSourceDisplay.prototype.name = function (name, replacement) {
    var replacing = this.node.querySelector('.templateLabel');
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
...
TelemClockSourceDisplay.prototype.units = function (val) {
    var units = this.node.querySelector('.templateUnits');
    if (units != null && units != undefined) {
        units.innerHTML = val;
    }
...
// -----  End Telemetry Display --------------------------------------------------------------
TelemDependentDisplay = function (component) {
    var node = null;
    var obj = this;
    var currentVal;
    var currentAlarm;
    ...
    if ((obj.node.getAttribute("data-device") != null) && (obj.node.getAttribute("data-sensor") != null)) {
        if (!checkTelemetryHandlerForElement(obj.node)) {
            registerObject(createTelemObject(obj, obj.node.getAttribute("data-device"), obj.node.getAttribute("data-ctrlSensor"), updateCustom, updateCustomLimit, updateCustomNoData));
            if (alarmIncludedFlag && !checkAlarmHandlerForElement(obj.node)) {
                registerAlarmObject(createAlarmObject(obj, obj.node.getAttribute("data-device"), obj.node.getAttribute("data-ctrlSensor"), updateAlarm, updateCustomNoData, updateMode));
            }
        }
    }
};

TelemDependentDisplay.prototype.onmousedown = function (event) {
    if (document.getElementById('popupMenu') != null && document.getElementById('popupMenu') != undefined) {
        var popup = this.node.getAttribute('data-nopopup');
        if (popup == "true") {
            return;
...
TelemDependentDisplay.prototype.refresh = function (val) {
    if (val == null || val == undefined) { return; }
    if (val == "reset_display") val = "---";
   // if (this.node.querySelector('.templateValue').innerHTML != val)
   //     this.node.querySelector('.templateValue').innerHTML = val;

    var cntrlr = this.node.getAttribute("data-ctrlsensor");
    if (cntrlr != null && cntrlr != undefined) {
        switch (cntrlr) {
            case "DPT_A_MODE":
                if (val.toUpperCase().indexOf("A") != -1) {
                    var display = document.getElementById("desired_dpt");
                    if (display != null && display != undefined) {
                        display.setAttribute("data-state", "disabled");
                    }
                    display = document.getElementById("desired_a");
                    if (display != null && display != undefined) {
                        display.setAttribute("data-state", "active");
                    }
                } else if (val.toUpperCase().indexOf("DPT") != -1)
                {
                    var display = document.getElementById("desired_a");
                    if (display != null && display != undefined) {
                        display.setAttribute("data-state", "disabled");
                    }
                    display = document.getElementById("desired_dpt");
                    if (display != null && display != undefined) {
                        display.setAttribute("data-state", "active");
                    }
                }
...
TelemDependentDisplay.prototype.refreshAlarm = function (val) {
    if (val == "DISABLED" || val == "0") {
        this.node.querySelector('.templateLabel').style.color = "orange";
        this.node.setAttribute("data-alarmstate", "disabled");
    } else if (val == "ENABLED" || val == "1") {
...
TelemDependentDisplay.prototype.refreshMode = function (val) {
    this.node.setAttribute('data-mode', val);
};

TelemDependentDisplay.prototype.limit = function (newVal) {
    if ((newVal == "RED_LIMIT_HIGH") || (newVal == "RED_LIMIT_LOW")) {
        this.node.querySelector('.templateValue').style.background = "#ff0000";
        this.node.querySelector('.templateValue').style.color = "#2e2e2e";
    }
...
TelemDependentDisplay.prototype.name = function (name, replacement) {
    var replacing = this.node.querySelector('.templateLabel');
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
...
TelemDependentDisplay.prototype.units = function (val) {
    var units = this.node.querySelector('.templateUnits');
    if (units != null && units != undefined) {
        units.innerHTML = val;
    }
...
// -----------------------------------------------------------------------------
ModeDependentButton = function (component) {
    var node = null;
    var obj = this;
    var currentVal = 1;
    var buttonID = null;
...
ModeDependentButton.prototype.refresh = function (val) { 
   
    if (this.node.id.toLowerCase().indexOf(val.toLowerCase()) != -1) {
        var btns = document.getElementsByClassName("ModeDependentButton");
        for (var i = 0; i < btns.length; i++) {
            btns[i].className = btns[i].className.replace(" ui-btn-active ui-state-persist", "");
        }
        this.node.className += " ui-btn-active ui-state-persist";
    }
    if (val.toUpperCase() == "WP") {
        document.getElementById('WPModeDisplay').setAttribute("data-state", "active");
        document.getElementById('HDModeDisplay').setAttribute("data-state", "disabled");
        var btn = document.getElementById("tab_wpMode");
        btn.className = btn.className + " ui-btn-active";
        btn = document.getElementById("tab_hdMode");
        btn.className = btn.className.replace(" ui-btn-active", "");
    } else {
        document.getElementById('HDModeDisplay').setAttribute("data-state", "active");
        document.getElementById('WPModeDisplay').setAttribute("data-state", "disabled");
        var btn = document.getElementById("tab_hdMode");
        btn.className = btn.className + " ui-btn-active";
        btn = document.getElementById("tab_wpMode");
        btn.className = btn.className.replace(" ui-btn-active", "");
    }
};

ModeDependentButton.prototype.refreshAlarm = function (val) {
};

ModeDependentButton.prototype.refreshMode = function (val) {
};


ModeDependentButton.prototype.limit = function (newVal) {
};

ModeDependentButton.prototype.name = function (name, replacement) {
    var replacing = null;
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
...
ModeDependentButton.prototype.units = function (val) { };
//--------------------------------------------------------------------------------------------------------------------------------------------
// -----------------------------------------------------------------------------
TelemDependentButton = function (component) {
...
TelemDependentButton.prototype.refresh = function (val) {

    if (this.node.getAttribute("data-device").toUpperCase() == "AP")
    {
        switch(this.node.getAttribute("data-sensor"))
        {
            case "AP_MODE":
                if (val.toUpperCase() == "WP") {
                    document.getElementById('WPModeDisplay').setAttribute("data-state", "active");
                    document.getElementById('HDModeDisplay').setAttribute("data-state", "disabled");
                    document.getElementById("current_mode_div").innerHTML = "WP Mode";
                    document.getElementById("modeBtn").innerHTML = "HD Mode";
                } else {
                    document.getElementById('HDModeDisplay').setAttribute("data-state", "active");
                    document.getElementById('WPModeDisplay').setAttribute("data-state", "disabled");
                    document.getElementById("current_mode_div").innerHTML = "HD Mode";
                    document.getElementById("modeBtn").innerHTML = "WP Mode";
                }
                break;
            case "RUN_STATE":
                var btn = document.getElementById("btn_run");
                if (btn != null && btn != undefined) {
                    if (val == 1 || val == true || val == "STOP") {
                        btn.innerHTML = "STOP";
                    } else {
                        btn.innerHTML = "RUN";
                    }
                ...
                break;
            case "LEAP_STATE":
                var btn = document.getElementById("btn_leap");
                if (btn != null && btn != undefined)
                {
                    if (val == 1 || val == true || val == "OFF") {
                        btn.innerHTML = "LEAP OFF";
                    } else {
                        btn.innerHTML = "LEAP";
                    }
                }
                break;
            case "DESIRED_MOTOR_SPEED":
                var oldSpeed = speed;
                document.getElementById('speedBtn').innerHTML = oldSpeed + " Speed";
                if (oldSpeed == "Full") {
                    speed = "Half";
                } else if (oldSpeed == "Half") {
                    speed = "Full";
                }

                var speedDependents = document.getElementsByClassName('calspeed');

                for (var i = 0; i < speedDependents.length; i++) {
                    if (speedDependents[i].className.indexOf("hidden") != -1) {
                        speedDependents[i].className = speedDependents[i].className.replace(" hidden", "").trim();
                    } else {
                        speedDependents[i].className += " hidden";
                    }
                    var x = speedDependents[i].className;
                }
                var text = speed + " Speed";
                document.getElementById('calibration_header_type').innerHTML = text;
                document.getElementById('cal_2_speed').innerHTML = text;
                document.getElementById('cal_3_speed').innerHTML = text;
                document.getElementById("current_desired_speed_div").innerHTML = text;
                document.getElementById("speedBtn").innerHTML = oldSpeed + " Speed";
                break;
            case "MOTOR_SPEED":
                /*
                var oldSpeed = speed;
                document.getElementById('speedBtn').innerHTML = oldSpeed + " Speed";
                if (oldSpeed == "Full") speed = "Half";
                else if (oldSpeed == "Half") speed = "Full";
...
TelemDependentButton.prototype.refreshAlarm = function (val) {
};

TelemDependentButton.prototype.refreshMode = function (val) {
};


TelemDependentButton.prototype.limit = function (newVal) {
};

TelemDependentButton.prototype.name = function (name, replacement) {
    var replacing = null;
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
...
TelemDependentButton.prototype.units = function (val) { };

// -----------------------------------------------------------------------------
TelemDependentSelect = function (component) {
...
TelemDependentSelect.prototype.refresh = function (val) {
    if (val == null || val == undefined)
    {
        return;
    }
    for (var i = 0; i < this.node.options.length; i++) {
        if (val == this.node.options[i].value) {
            this.node.options[i].selected = true;
            this.node.selectedOptions = this.node.options[i];
            $("#" + this.node.id).selectmenu('refresh');
            break;
        }
    }
    return;
...
TelemDependentSelect.prototype.refreshAlarm = function (val) {
};

TelemDependentSelect.prototype.refreshMode = function (val) {
};


TelemDependentSelect.prototype.limit = function (newVal) {
};

TelemDependentSelect.prototype.name = function (name, replacement) {
    var replacing = null;
    if (replacing != null && replacing != undefined) {
        this.node.setAttribute("data-label", replacement);
        replacing.innerHTML = replacement;
...
TelemDependentSelect.prototype.units = function (val) { };
...
// Register all leds
(function(){
    var leds = document.getElementsByClassName("swd");
    for(var i = 0, l = leds.length; i < l; i++){
        new SwdDisplay(leds[i]);
    }
})();
...
(function(){
    ...
    for(var i = 0, l = moduleButtons.length; i < l; i++){
        var tempModuleButton = new ModuleButton(moduleButtons[i], i);
    }
})();
...
(function () {
    var displays = document.getElementsByClassName("telemDprDptDisplay");
    for (var i = 0, l = displays.length; i < l; i++) {
        var tempTelemDisp = new TelemDprDptDisplay(displays[i]);
    }
})();

// Register all Ground Table items
(function () {
    var displays = document.getElementsByClassName("gndTable");
    for (var i = 0, l = displays.length; i < l; i++) {
        var tempGroundTable = new GroundTable(displays[i]);
    }
})();

// Register all Telemetry Displays
(function(){
    var displays = document.getElementsByClassName("TelemClockSourceDisplay");
    for(var i = 0, l = displays.length; i < l; i++){
        var tempTelemDisp = new TelemClockSourceDisplay(displays[i]);
    }
})();


// Register all Telemetry Buttons
function registerTelemButtons() {
    var displays = document.getElementsByClassName("telemButton");
    for (var i = 0, l = displays.length; i < l; i++) {
        new TelemButton(displays[i]);
    }
}
registerTelemButtons();
...
(function () {
    ...
    for (var i = 0, l = btns.length; i < l; i++) {
        new ModeDependentButton(btns[i]);
    }
})();

(function () {
    var displays = document.getElementsByClassName("TelemDependentDisplay");
    for (var i = 0, l = displays.length; i < l; i++) {
        new TelemDependentDisplay(displays[i]);
    }
})();

(function () {
    var btns = document.getElementsByClassName("TelemDependentButton");
    for (var i = 0, l = btns.length; i < l; i++) {
        new TelemDependentButton(btns[i]);
    }
})();
...
// Register all inverse swds
(function () {
    var swds = document.getElementsByClassName("inverseswd");
    for (var i = 0, l = swds.length; i < l; i++) {
        new InverseSwdDisplay(swds[i]);
    }
...
(function () {
    var selects = document.getElementsByClassName("telemSelect");
    for (var i = 0; i < selects.length; i++) {
        new TelemDependentSelect(selects[i]);
    }
})();
