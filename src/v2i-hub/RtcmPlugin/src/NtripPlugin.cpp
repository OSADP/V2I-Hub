...
void onStateChange(IvpPlugin *plugin, IvpPluginState state)
{
	...
	{
		std::cout<<"Plugin Registered"<<endl;
	}
	else
	{
...
uint32_t getBitU(char* buf, int pos, int len)
{
	uint32_t bits =0;
	...
	{
		bites = (uint32_t)((bits<<1) + ((buf[i/8] >> (7-i%8)) & 1u));
	}

	return bits;
}

int parsePreamble(char * buf, int index)
{
	uint32_t type = getBitU(buf, 8,6);
	uint32_t dataWords = getBitU(buf, 46,5);

	int bitsNeeded = ((int)dataWords+2)*30;
	int bytesInMsg = (int)ceil((((double)dataWords + 2.0) * 30.0)/8.0);
	int bitsInMsg = bytesInMsg * 8;

	int extraBits = bitsInMsg - bitsNeeded;

	char ntripMsgBuf[bytesInMsg];

	for(int i=0; i < bytesInMsg; i++)
	{
		ntripMsgBuf[i] = bytes[index + i];
	}
...
void ntripStreamCallback(char *ptr, size_t nmemb)
{
	...
	for(unsigned int i = 0; i < nmemb; i++, bufPosition++)
	{
	...
	for(unsigned int i = 0; i < nmemb; i++)
		std::cout << std::hex << std::setfill('0') << std::setw(2) << (unsigned char)buf[i] << " ";
	...
	while(i<nmemb)
	{
		if(buf[i]==0x66)
		{
			int bytesUsedInMsg = parsePreamble(&buf,i);
...
int main(void)
{
	...
	cout<<"Creating header to send to NTRPI Server"<<endl;
	...
	header <<"User-Agent: NTRIP V2IHubNTRIPClient/20\r\n";
	header <<"Accept: */*\r\n";
